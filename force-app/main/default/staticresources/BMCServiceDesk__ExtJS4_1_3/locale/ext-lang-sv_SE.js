...
Ext.onReady(function() {
    ...
    override: 'Ext.util.Format'
		...
		Ext.util.Format.date = function (v, format) {
			return originalDate(v,format).replace('am', 'fm').replace('pm', 'em').replace('AM', 'FM').replace('PM', 'EM');
		}
		var originalParse = Ext.Date.parse;
		Ext.Date.parse = function (input, format, strict) {
			return originalParse(input.replace('em', 'pm').replace('fm', 'am').replace('EM', 'PM').replace('FM', 'AM'), format, strict);
		}
	});
