ArcGIS.plotHelpers = {
					...
					if (getProperty(response, 'data.error')) {
						...
						if (layerData.url) {
							...
							if (layerData.layerType) {
								...
								window.VueEventBus.$emit('remove-layer', layerId);
								MAToastMessages.showError({
									message: (layerData.type ? layerData.type + ': ' : '') + 'Unsupported layer type. Please share this error and details with Maps Support and your ArcGIS Online Administrator.',
									timeOut: 3000,
									extendedTimeOut: 0
								});
							}
							...
							window.VueEventBus.$emit('remove-layer', layerId);
							MAToastMessages.showError({
								message: 'No layer URL retrieved. Please share this error and details with Maps Support and your ArcGIS Online Administrator.',
								timeOut: 3000,
								extendedTimeOut: 0
							});
						}
        ...
        ArcGIS.plotHelpers.plotBasemapLayers(layerId, allLayersData.baseMap, webMapURL, refreshParams).then(function(response) {
			...
			ArcGIS.plotHelpers.plotOperationalLayers(layerId, allLayersData.operationalLayers, webMapURL, refreshParams).then(function(response) {
				...
				if (layerLegend.length > 0) {
					...
					dfd.resolve({
						success: false,
						error: 'No layers found in web map. You may need to edit the web map within ArcGIS Online. Please share this error and details with Maps Support and your ArcGIS Online Administrator.'
					});
				}
            });
        });
					...
					ArcGIS.ajaxRequest(requestData).then(function(response) {
						...
						if (response.success) {
							...
							if (responseData.error) {
								...
								MAToastMessages.showError({
									message: responseData.error.message + '. Unable to load layer &lt;' + layerTitle + '&gt;. Please share this error and details with Maps Support and your ArcGIS Online Administrator.',
									timeOut: 3000,
									extendedTimeOut: 0
								});
	...
	createLabelMarker: function(options) {
		...
		var labelMarkerURL = MAIO_URL + 'sfmapsapi.com/images/marker' +
