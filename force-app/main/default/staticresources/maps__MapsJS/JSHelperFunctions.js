...
function ChangeOwner_Finish(req)
{
    ...
    var changeOwnerMessage = MAToastMessages.showLoading({message:'Change Owner Status',subMessage:'Updating Records...',timeOut:0,extendedTimeOut:0});
    if(MA.isMobile) {
        MALayers.hideModal();
    }
    ...
    recQue.drain = function() {
        ...
        else
        {
            if(MA.isMobile) {
                MALayers.hideModal();
                MAToastMessages.hideMessage(changeOwnerMessage);
                MAToastMessages.showWarning({message:'Change Owner Results',subMessage:$('#changeowner-options-wrapper').data('successCount') + ' records updated, ' + $('#changeowner-options-wrapper').data('failureCount') + ' failure(s).'})
            }
            else {
                MAToastMessages.showWarning({message:'Change Owner Results',subMessage:$('#changeowner-options-wrapper').data('successCount') + ' records updated, ' + $('#changeowner-options-wrapper').data('failureCount') + ' failure(s).'})
                setTimeout(function() {
                ...
                if (!response.success) {
                    ...
                    if (!MASystem.Organization.DisableClick2CreateMarkers) {
                        ...
                        var marker = new google.maps.Marker({
                            ...
                            icon: {
                                url: MAIO_URL+ '/images/marker?color=ff8800&forlegend=false&icon=Favorite',
...
function NewTask(recordIdMap)
{
    ...
    $popup.find('.search-table-view').empty().html('<li class="table-view-cell">' + window.MASystem.Labels.MA_SearchAboveEllipsis + '</li>');
        ...
        searchTimeout = setTimeout(function() {
            ...
            searchUsers(searchTerm).then(function(res) {
                ...
                if(res.success) {
                    ...
                    if(userDataLength === 0) {
                        //show no results
                        $searchResults.html('<li class="table-view-cell">' + window.MASystem.Labels.MA_No_Results + '</li>');
                    }
                    else {
                        $.each(userData, function (index, data) {
                            resultHTML += '<li class="table-view-cell ownerSuccess" data-id="'+htmlEncode(data.Id)+'">'+htmlEncode(data.Name)+'</li>';
...
function NewEvent(recordIdMap)
{
    ...
    //clear previous search history
    $popup.find('.search-table-view').empty().html('<li class="table-view-cell slds-media slds-listbox__option slds-listbox__option_plain slds-media_small slds-media_center">' + window.MASystem.Labels.MA_SearchAboveEllipsis + '</li>');
    $popup.find('.searchDynamicInput ').val('').removeAttr('data-id');
    //showLoading($('#newtask-select-grid-wrapper'), 'Loading Users...');
        ...
        searchTimeout = setTimeout(function() {
            ...
            searchUsers(searchTerm).then(function(res) {
                ...
                if(res.success) {
                    ...
                    if(userDataLength === 0) {
                        //show no results
                        $searchResults.html('<li class="table-view-cell slds-media slds-listbox__option slds-listbox__option_plain slds-media_small slds-media_center">' + window.MASystem.Labels.MA_No_Results + '</li>');
                    }
                    else {
                        $.each(userData, function (index, data) {
