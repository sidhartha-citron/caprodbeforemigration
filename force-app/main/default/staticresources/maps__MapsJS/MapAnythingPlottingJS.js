...
var Plotting = {
    ...
    updateQueryInfo: function ($plottedQuery, callback) {
        ...
        if (queryData.numGeocodeRequestsNeeded > 0 || badAddresses.length > 0) {
            ...
            infoHTML += "<div>" + numNeeded + " " + MASystem.Labels.MA_geocodes_completed + ". " + geoErrorsCountHTML + "<span onclick='Plotting.showBadGeocodes(\"" + queryData.qid + "\");' class='geocode link'>" + queryData.numFailedGeocodes + " " + MASystem.Labels.MA_bad_address + "</span>.</div>";
        }
        ...
        if ($plottedQuery.find('.renderButtons-button.markers').is('.on')) {
            ...
            if (/live/i.test(queryData.layerType) && !MA.isMobile) {
                ...
                if (Array.isArray(liveCriteriaUnmet) && isNum(visibleCount)) {
                    infoDivText += "</br>" + visibleCount + " visible. <span onclick='Plotting.showLiveCriteriaInfo(\"" + queryData.qid + "\");' class='geocode link'>" + liveCriteriaUnmet.length + ' not visible</span>';
                }
            }
    ...
    showBadGeocodes: function (qid) {
        ...
        for (var b = 0, len = badAddresses.length; b < len; b++) {
            ...
            html += '<tr class="missingAddress" data-id="' + record.Id + '">' +
                ...
                '<td class="MAbutton button-clear button-small button-blue" onclick="Plotting.MissingAddress_DropPin(this,{qid:\'' + queryData.qid + '\',recordId:\'' + record.Id + '\'})">Drop Pin</td>' +
                '</tr>';
    ...
    marker_Click: function (options, callback) {
        ...
        function tabCreateActivate(event, ui) {
            ...
            if (panel.filter("div[id^='pane-weather']").length > 0 && !$tooltipContent.find('#tab-weather').data('loaded')) {
                ...
                var $MarkerLayerRelatedList = $tooltipContent.find("#ajax-" + marker.qid).html(MASystem.Labels.MA_Loading + '...');
