...
/*Use to redirect to any other page in MALive Product
...
*/
function redirectPage(action, sumId, extraActions) {
    
    if(isValueEmptyProto(sumId)){
        sumId = typeof summaryId !== 'undefined' && !isValueEmptyProto(summaryId) ? summaryId : sumId;
    } 
    ...
    switch (action) {
        case "new":
            lightBaseURL += '/apex/malive__MALiveDailySummary';
            break;
        case "cancel":
            lightBaseURL += '/apex/malive__MALiveSummariesDashboard';
            break;
        case "associations":
            lightBaseURL += '/apex/malive__MALiveStopAssociations';
            break;
        case "open":
            if(!isValueEmptyProto(sumId)){
                lightBaseURL += '/apex/malive__MALiveDailySummary?id=' + sumId + (isValueEmpty(extraActions) ? '' : extraActions);
            }
            else {
        ...
        case "clone":
            if(!isValueEmptyProto(sumId)) {
                lightBaseURL += '/apex/malive__MALiveDailySummary?id=' + sumId + '&clone=true' + (isValueEmpty(extraActions) ? '' : extraActions);
            }
            else {
        ...
        case "map":
            if(lightning) {
                lightBaseURL += (sumId ? '/apex/sma__MapAnything?layerid=' + sumId : '/apex/sma__MapAnything') + (isValueEmpty(extraActions) ? '' : extraActions);
                //sforce.one.navigateToURL(lightBaseURL, false);
                window.open(lightBaseURL, '_blank');
            }
            else {
                var pageLoc = (sumId ? '/apex/sma__MapAnything?layerid=' + sumId + '&retUrl=' : '/apex/sma__MapAnything?retUrl=') + (isValueEmpty(extraActions) ? '' : extraActions);
                var retUrl = (sumId ? '/apex/malive__MALiveDailySummary?id=' + sumId : '/apex/malive__MALiveDailySummary');
                pageLoc += encodeURI(retUrl);
                window.open(pageLoc, '_blank');
            }
            
            if(isValueEmptyProto(sumId)){
                messageText = 'You cannot open a summary in MapAnything that has no associated id.';
                alertType = 'error';
                title = null;
                checkForMessages();
            }
