...
import {
	...
	getUnsupportedLexTagsFoundInText,
	getEditorBodyText,
...
let data = {
	...
	needToConfirmAppendUnsubscribe: false,
	...
	unsupportedLexTagsFoundInBody: undefined,
	unsupportedLexTagsFoundInSubject: undefined,
	...
	__onDispatch(action) {
		...
		switch (action.type) {
			...
			case a.TEMPLATE_RECEIVED: {
				...
				data.unsupportedLexTagsConfirmed = null;
				data.unsupportedLexTagsFoundInBody = null;
				data.unsupportedLexTagsFoundInSubject = null;
				if (isNaN(data.template.id)) {
					data.unsupportedLexTagsConfirmed = false;
					data.unsupportedLexTagsFoundInBody = getUnsupportedLexTagsFoundInText(data.template.html);
					data.unsupportedLexTagsFoundInSubject = getUnsupportedLexTagsFoundInText(data.template.subject);
				}
			...
			case a.TEMPLATE_BODY_CHANGED:
				...
				data.unsupportedLexTagsFoundInBody = action.unsupportedLexTagsFound;
				data.unsupportedLexTagsConfirmed = !action.needsLexTagConfirming;
			...
			case a.SUBJECT_CHANGED:
				...
				data.unsupportedLexTagsFoundInSubject = null;
				if (isNaN(data.template.id)) {
					data.unsupportedLexTagsFoundInSubject = getUnsupportedLexTagsFoundInText(action.subject);
				}
			...
			case a.CONFIRM_POLICY:
				...
				sendEmail.call(this);
				break;
			case a.CANCEL_APPEND_UNSUBSCRIBE:
				data.needToConfirmAppendUnsubscribe = false;
				this.__emitChange();
				break;
			case a.CONFIRM_APPEND_UNSUBSCRIBE:
				data.needToConfirmAppendUnsubscribe = false;
				data.hasConfirmAppendUnsubscribe = true;
				sendEmail.call(this);
				break;
...
function appendUnsubLinkToTemplateIfNecessary(calledOnTabSwitch) {
	...
	if (data.recipients.withEmail().length > unsubscribeThreshold) {
		...
		if (shouldAppendUnsubscribe(html)) {
			html = html.concat(unsubscribeHtmlFooter);
		}
		if (shouldAppendUnsubscribe(text)) {
			if (window.hasHmlEnabled) {
...
function validatePermissionPolicy() {
	...
	if (recipients.length > 10 && !data.hasConfirmedPermissionPolicy) {
		...
		data.needToConfirmAppendUnsubscribe = false;
		if (shouldAppendUnsubscribe(bodyText)) {
			data.needToConfirmAppendUnsubscribe = true;
		}
		return false;
	}
...
function shouldAppendUnsubscribe(body) {
	return body !== null &&
			(body.indexOf('%%unsubscribe%%') === -1 && body.indexOf('{{Unsubscribe}}') === -1) && 
			(!body.match(/\%\%email_preference_center_?[0-9]*\%\%/g) && !body.match(/\{\{EmailPreferenceCenter_?[0-9]*\}\}/g))
}
