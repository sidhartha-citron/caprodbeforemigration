...
export default class EngageMessages extends React.Component {
	...
	renderAlerts() {
		let {
			...
			unsupportedLexTagsFoundInBody,
			unsupportedLexTagsFoundInSubject,
			userFullName,
		...
		let badTagsInEmail, badLexTagsInEmail;
		...
		unsupportedLexTagsFoundInBody = isArrayWithElements(unsupportedLexTagsFoundInBody) ? unsupportedLexTagsFoundInBody : [];
		unsupportedLexTagsFoundInSubject = isArrayWithElements(unsupportedLexTagsFoundInSubject) ? unsupportedLexTagsFoundInSubject : [];
		badLexTagsInEmail = unsupportedLexTagsFoundInBody.concat(unsupportedLexTagsFoundInSubject);
		...
		if ((badTagsInEmail.length > 0 && !sendOnBehalfOf.allOwnersInPardot) || badLexTagsInEmail.length > 0) {
			let allBadTagsInEmail = badTagsInEmail.concat(badLexTagsInEmail);
			alerts.push(
				<UnsupportedTagsAlert unsupportedTagsFound={allBadTagsInEmail} key={alerts.length}/>
			);
		}
	...
	renderModal() {
		let {
			...
			needToConfirmAppendUnsubscribe,
		...
		if (isEmailBlockedForAccount) {
			...
			return <PolicyModal appendUnsubscribe={needToConfirmAppendUnsubscribe} />;
