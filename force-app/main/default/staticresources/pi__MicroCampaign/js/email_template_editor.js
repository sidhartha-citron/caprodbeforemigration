...
import {
	...
	unsupportedLexTagIdentifier,
	unsupportedLexTagSvgIdPrefix,
	unsupportedLexTagBufferIdentifier,
	types
...
import {AttachmentContainerEditTabClass, SendOnBehalfOfOptions, SupportedTagsInPardot, UnsupportedUserTags} from './constants';
...
function unsupportedTagCheckOnChange(event = null) {
	...
	let unsupportedLexTagsFound = null;
	let templateId = EngageEmailStore.get().template.id;

	let needsLexTagConfirming = !EngageEmailStore.get().unsupportedLexTagsConfirmed;
	if (isNaN(templateId)) {
		let unsupportedLexTagsFoundInBody = EngageEmailStore.get().unsupportedLexTagsFoundInBody;

		unsupportedLexTagsFound = getUnsupportedLexTagsFoundInText(data);
		if (!identicalArrays(unsupportedLexTagsFound, unsupportedLexTagsFoundInBody)) {
			handleUnsupportedLexTagFeedback(unsupportedLexTagsFound, editor);
			stripCkEditorBsTitleElement();

			//user added a new unsupported tag that needs to be confirmed
			if (unsupportedLexTagsFound.length >= unsupportedLexTagsFoundInBody.length) {
				needsLexTagConfirming = true;
			}
		}
	}
	...
	Dispatcher.dispatch({
		...
		needsTagConfirming,
		unsupportedLexTagsFound,
		needsLexTagConfirming
	});
}
...
function handleUnsupportedLexTagFeedback(unsupportedLexTagsFound, editor) {
	let bodyBeforeBookmark = editor.document.getBody().getHtml();
	//save cursor spot so we can restore after replacing body
	let bookmark = editor.getSelection().createBookmarks(true);
	let body = editor.document.getBody().getHtml();
	...
	//remove highlight divs -- will add back later if necessary.
	body = removeLexHighlightElementsFrom(body);

	//wrap current unsupported tags in body with highlighted divs
	if (unsupportedLexTagsFound !== null) {
		addLexHighlightDivs(unsupportedLexTagsFound, body);
	}
	//restore cursor location
	editor.getSelection().selectBookmarks(bookmark);
}
...
export function removeLexHighlightDivs() {
	let body = getEditorBodyText();
	body = removeLexHighlightElementsFrom(body);
	$(ckeditorClass).contents()[0].body.innerHTML = body;
}
...
function removeLexHighlightElementsFrom(body) {
	let unWrappedBody = $('<div />').append(body)
	//remove parent div (the highlighting)
		.find(`.${unsupportedLexTagIdentifier}`).children().unwrap().end().end()
		//remove warning icon
		.find(`svg[id^="${unsupportedLexTagSvgIdPrefix}"]`).each((index, node) => {
			node.parentNode.removeChild(node)
		}).end()
		//remove any empty spans (just in case)
		.find(`.${unsupportedLexTagIdentifier}:empty`).each((index, node) => {
			node.parentNode.removeChild(node)
		}).end();

	// If template locking is enabled, remove the buffer span
	if (unWrappedBody.html().includes('locked="1"')) {
		unWrappedBody
			//remove parent span - if there are child elements
			.find(`.${unsupportedLexTagBufferIdentifier}`).children().unwrap().end().end()
			//remove parent span - if there are only text children
			.find(`.${unsupportedLexTagBufferIdentifier}`).each(function() {
				$(this).replaceWith(this.childNodes)
			}).end()
			// remove empty buffer span
			.find(`.${unsupportedLexTagBufferIdentifier}`).each((index, node) => {
				node.parentNode.removeChild(node)
			}).end();
	}
...
export function addLexHighlightDivs(tags, body) {
	let { template } = EngageEmailStore.get();

	if(!isNaN(template.id)) {
		return;
	}
	...
	let svgPath = warningIconMarkup();
	let templateContainsLockedRegions = body.includes('locked="1"');

	for (let i = 0; i < dedupedTags.length; i++) {
		let span = getLexHighlightSpan(i, dedupedTags, svgPath, templateContainsLockedRegions);
		body = body.replace(new RegExp(dedupedTags[i], 'g'), span);
	}
	//replace ckeditor body with new body
	$(ckeditorClass).contents()[0].body.innerHTML = body;
...
function getLexHighlightSpan(i, dedupedTags, svgPath, templateContainsLockedRegions) {
	let span =
		`<span contenteditable="false" class="${unsupportedLexTagIdentifier}" title="Unsupported Lex Variable Tag">` +
		`<svg id="${unsupportedLexTagSvgIdPrefix + i}" width="1em" height="1em" viewBox="0 0 24 24">` +
		svgPath +
		'</svg>' +
		dedupedTags[i] +
		'</span>';
	...
	if (templateContainsLockedRegions) {
		span += `<span class="${unsupportedLexTagBufferIdentifier}"></span>`;
	}
...
export function getUnsupportedLexTagsFoundInText(text) {
	if (!text) {
		return null;
	}
	let tags = parsePardotVariableTags(text);
	return tags.filter(tag => !SupportedTagsInPardot.includes(tag.toLowerCase()));
}
...
EmailTemplateEditor.prototype.setTemplate = function(newTemplate)
{
	...
	return new Promise((resolve, reject) => {
		...
		let { soboEnabled, sendOnBehalfOf, template } = EngageEmailStore.get();
		...
		if (isNaN(template.id)) {
			let unsupportedLexTagsFound = getUnsupportedLexTagsFoundInText(this.template.html);
			if (unsupportedLexTagsFound) {
				handleUnsupportedLexTagFeedback(unsupportedLexTagsFound, this.editor);
			}
		}
