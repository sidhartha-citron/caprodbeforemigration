...
function _getX(ev, isEndPoint, config) {
	...
	var delta = date - scheduler._min_date_timeline - (date.getTimezoneOffset() - scheduler._min_date_timeline.getTimezoneOffset()) * 60000;
	...
	if (delta > 0){
		...
		if (!round_position) {
			delta = date - column_date - (date.getTimezoneOffset() - column_date.getTimezoneOffset()) * 60000;
	...
	for (var i=0; i<this.y_unit.length; i++){
		...
		if (this.render == "cell"){
			...
			for (var j=0; j < scheduler._cols.length; j++){
				...
				{
					// original code
					//html+="<td class='dhx_matrix_cell "+scheduler.templates[this.name+"_cell_class"](evs[i],this._trace_x[j],this.y_unit[i])+"' style='width:"+(scheduler._cols[j])+"px'><div style='width:auto'></div></td>";

					// added by Shai
					var innerCellText = scheduler.templates[this.name+"_cell_value"](this._trace_x[j],this.y_unit[i]);
					if (innerCellText)
						html += "<td class='dhx_matrix_cell custom_cell" + scheduler.templates[this.name + "_cell_class"](evs[i], this._trace_x[j], this.y_unit[i]) + "' style='width:" + (scheduler._cols[j]) + "px'>" + scheduler.templates[this.name + "_cell_value"](this._trace_x[j], this.y_unit[i]) + "</td>";
					else
		...
		for (var j=0; j<units.length; j++) {
			...
			for (var i=0; i<dates.length; i++) {
				...
				for (var k=0; k<zones.length; k += 2) {
					...
					start_date = scheduler.date.add(start_date, start_date.getTimezoneOffset() - date.getTimezoneOffset(), "minute");

					var end_date = new Date(+date + zone_end * 60 * 1000);
					end_date = scheduler.date.add(end_date, end_date.getTimezoneOffset() - date.getTimezoneOffset(), "minute");
