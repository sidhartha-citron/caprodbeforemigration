...
var Worker_MAIO_URL = 'https://api.mapanything.io';
...
function processRecords (data,callback) {
    /**
        ...
        6. isIE?
        7. legendInfo
        8. ImageServerEnabled
        9. addressProxLimitInfo
    */

    //grab the records and queryInfo
    var ImageServerEnabled = data.ImageServerEnabled || false;
    ...
    Worker_MAIO_URL = data.MAIO_URL || 'https://api.mapanything.io';
    // upate the url with icon info
    Worker_MAIO_URL = Worker_MAIO_URL + '/services/images/marker';
    ...
    //always use the imageserver if IE, otherwise IE cannot handle markers
    if(isIE) {
        ImageServerEnabled = true;
    }
    ...
    for(var i = 0, len = records.length; i < len; i++) {
        ...
        // BK - Added to support current scheduling module. This can be removed after the Spring '18 release of sma.
        ...
        record.baseObjectId = queryRecord.sma__BaseObject__c;
        ...
        var tooltip1 = '';
        try {
            tooltip1 = workerFormatTooltip(record,tooltipMeta[0],true);
            ...
            tooltip1 = workerGetProperty(record,queryRecord.Tooltip1__c) || 'N/A';
        }
        ...
        try {
            var formattedAddress = workerGetFormattedAddress(record,addressFields);
        ...
        function processColorAssignentsForDynamicLabel()
        {
            ...
            var picklistValue = String(workerGetProperty(record,queryRecord.PicklistField__c)) || '';
            ...
            if(!foundMatch) {
                ...
                if(automaticAssign) {
                    ...
                    if(autoAssignMap[picklistValue]) {
                        ...
                        // shape can be static as specified in the advancedOptions or assigned randomly as per JIRA case: MAP-250 ('https://mapanything.atlassian.net/browse/MAP-250')
                        var shape;
                        ...
                        var nameValue = workerGetProperty(record,colorByPicklistField);
        ...
        if(markerAssignmentType == 'Static') {
            ...
            var urlColor = '';
            var imageURL = '';
            picklist1Value = String(workerGetProperty(record,picklist1Value)) || '';
            ...
            picklist2Value = String(workerGetProperty(record,picklist2Value)) || '';
        ...
        else if(markerAssignmentType == 'Dynamic-Label') 
        {
            ...
            var recordLabelValue = workerGetProperty(record,labelFieldToGet) || tooltip1;
...
function workerGetProperty(obj, prop, removeWorksapce) 
{
    ...
    if(removeWorksapce !== false)
    {
        ...
        if ( 'sma' == 'sma')
        {
            obj = workerRemoveNamespace(obj,'sma__');
...
function workerRemoveNamespace (obj,nameSpace) {
    try {
        var namespacePrefix = nameSpace || 'sma__';
        for(var key in obj) {
            var val = obj[key];
...
var workerLegendCompare = {
    ...
    date : function (options) {
        ...
        try {
            ...
            if (stringParts[0] == 'NEXT' || stringParts[0] == 'LAST') {
                ...
                if (!isNaN(parseInt(stringParts[1])) && stringParts.length == 3 && inArray) {
                    ...
                    else if (stringParts[0] == 'LAST' && picklistMoment.isSameOrBefore(moment().local().endOf(stringParts[2].substring(0,stringParts[2].length-1).toLowerCase()))) {
                        toMatches = true;
                    }
                }
        ...
        try {
            ...
            if (!fromIsDateLiteral && (picklistValue == '' || momentPickList.isAfter(momentStart) || (momentPickList.format('YYYY-MM-DD') == momentStart.format('YYYY-MM-DD') ) )) {
                   fromMatches = true;
               }
               if (!toIsDateLiteral && (picklistValue == '' || momentPickList.isBefore(momentEnd) || (momentPickList.format('YYYY-MM-DD') == momentEnd.format('YYYY-MM-DD') ) )) {
                   toMatches = true;
               }
        }
        catch(e){}
...
var MADateLiteralsObj = {
    ...
    */
       currentFiscalQuarterStartDate: null,
    currentFiscalQuarterEndDate: null,
    fiscalQuarters: null,
    findMatch: function(start, end) {
        var startYear = start.year();
        var endYear = end.year();
        var quarterMatchFound = false;
        for (var i = 0; i < this.fiscalQuarters.length; i++) {
            var quarter = this.fiscalQuarters[i];
            var quarterStartMomentDate = moment(`${startYear.toString()}-${quarter.StartMonth.toString()}-${quarter.StartDay.toString()}`);
            var quarterEndMomentDate = moment(`${endYear.toString()}-${quarter.EndMonth.toString()}-${quarter.EndDay.toString()}`).endOf('day');

            if (end.isBetween(quarterStartMomentDate, quarterEndMomentDate, 'days', '[]') && start.isBetween(quarterStartMomentDate, quarterEndMomentDate, 'days', '[]')) {
                quarterMatchFound = true;

                return {
                    start: quarterStartMomentDate.valueOf(),
                    end: quarterEndMomentDate.valueOf()
                }
            }
        }
    },
    ...
    'NEXT N FISCAL QUARTERS': function(options) {
        this.fiscalQuarters = this.fiscalQuarters || MAWorkerGetProperty(options, 'FiscalYearSettings.thisYearsQuarters');
        this.currentFiscalQuarterStartDate = this.currentFiscalQuarterStartDate || MAWorkerGetProperty(options, 'FiscalYearSettings.currentFiscalQuarterStartDate');
        this.currentFiscalQuarterEndDate =  this.currentFiscalQuarterEndDate || MAWorkerGetProperty(options, 'FiscalYearSettings.currentFiscalQuarterEndDate');
        var dateFormat = MAWorkerGetProperty(options, 'FiscalYearSettings.dateFormat');

        var startTmp = moment(this.currentFiscalQuarterStartDate, dateFormat).add(options.value, 'quarter').startOf('day');
        var endTmp = moment(this.currentFiscalQuarterEndDate, dateFormat).add(options.value, 'quarter').endOf('day');
        
        return this.findMatch(startTmp, endTmp);
    },
    'LAST N FISCAL QUARTERS': function(options) {
        ...
        var startTmp = moment(this.currentFiscalQuarterStartDate, dateFormat).subtract(options.value, 'quarter').startOf('day');
        var endTmp = moment(this.currentFiscalQuarterEndDate, dateFormat).subtract(options.value, 'quarter').endOf('day');
...
function workerFormatTooltip(obj, toolTipObj, noLinks)
{
    ...
    if(toolTipObj.needsLink) {
        ...
        var rName = workerGetProperty(obj, toolTipObj.ActualFieldName);
    ...
    {
        var fieldValue = workerGetProperty(obj, toolTipObj.ActualFieldName);
