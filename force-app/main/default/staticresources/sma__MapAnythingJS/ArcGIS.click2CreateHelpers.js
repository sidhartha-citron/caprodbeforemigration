ArcGIS.click2CreateHelpers = {
			...
			if (response.success) {
				ArcGIS.plotHelpers.retrieveLayerInfo(featureLayerURL).then(function(response) {
					if (response.success) {
								...
								ArcGIS.plotHelpers.retrieveLayerInfo(featureLayerURL).then(function(response) {
									if (response.success) {
										...
										if(sublayerInfo.fields) {
											...
											console.error('Could not connect to the REST layer. Please check your ArcGIS REST layer URL.  No Fields Found.');
											return MAToastMessages.showError({
												message: 'Could not connect to the REST layer. Please check your ArcGIS REST layer URL.  No Fields Found.'
											});
										}
									}
						...
						$c2cLoading.remove();
						var errorMsg = response.error || 'Failed connection attempt to ArcGIS';
						console.error('Could not connect to the REST layer. Please check your ArcGIS REST layer URL. \n' + errorMsg);
						MAToastMessages.showError({
							message: 'Could not connect to the REST layer. Please check your ArcGIS REST layer URL. \n' + errorMsg,
							timeOut: 3000,
							extendedTimeOut: 0
						});
						dfd.resolve({
    ...
    getObjectsWithFieldSets: function(callbackFunction) {
        var dfd = jQuery.Deferred();
        ...
        var requestData = {
            ajaxResource: 'MAArcGISAPI',
            securityToken: MASystem.MergeFields.Security_Token,
            action: 'getObjectsWithFieldSets'
        };
		ArcGIS.ajaxRequest(requestData).then(function(response) {
			if (response.success) {
				//since we got a successful response we can pass our object info into our callback function and then execute it.
				//callbackFunction(response.objectInfo);
				dfd.resolve(response.objectInfo);
			}
        }, {
            buffer: false,
            escape: false,
    ...
    populateTemplates: function(templateData, callbackFunction) {
        ...
        $.each(templateData, function(i, rowData) {
            ...
            if (webMapLabel != undefined && webMapLabel != '') {
                ...
                templateRow.find('.webmap-active').prop('checked', isActive);
	...
	createRecordStep1: function(options) {
		...
		$objectPicklist.append('<option value="none">Loading...</option>');
		var $fieldSetSelect = $('#createArcLayerRecord .createrecord2-fieldset').empty();
		$fieldSetSelect.append('<option value="none">Loading...</option>');
		var $recordTypeSelect = $('#createArcLayerRecord .createrecord2-recordtype').empty();
		$recordTypeSelect.append('<option value="none">Loading...</option>');
		ArcGIS.click2CreateHelpers.getObjectsWithFieldSets().then(function(objectResponse) {
			...
			$.each(objectResponse, function(k, v) {
				objectString += '<option value="' + v.value + '">' + v.label + '</option>';
	...
	createRecordStep2: function(templates) {
		var $popup = $('#createArcLayerRecord');
		$popup.find(".createrecord-fieldset").html('Loading...');
		...
		$createrecord2.empty();
		var templateOptions = '<option value="default">--None--</option>';
	...
	populateFields: function() {
		...
		$.each(fieldMappings, function(k, v) {
			...
			var $fieldDiv = $('.ClickToCreateFormTable .fieldInput[data-field="' + fieldPath + '"]');
			var $field = $fieldDiv.find('.get-input');
			var rawFieldInputData = arcFieldInfo[featurePath];

			if ($fieldDiv.attr('data-type') == 'DATETIME' || $fieldDiv.attr('data-type') == 'DATE') {
				var dateFormat = formatUserLocaleDate({datepicker: true}).replace('yy', 'YYYY').replace('mm', 'MM').replace('dd', 'DD');
				var localMoment = moment(rawFieldInputData);
				localMoment.add(-1 * localMoment.utcOffset(), 'm');
				
				if (localMoment.isValid()) {
					$fieldDiv.find('.datepicker').val(localMoment.format(dateFormat));

					if (getProperty(v, 'locked', false)) {
						$fieldDiv.find('.datetimepicker-now-option').hide();
					}
					else {
						$fieldDiv.find('.datetimepicker-now-option').show();
					}

					if ($fieldDiv.attr('data-type') == 'DATETIME') {
						...
						$fieldDiv.find('.updateFields-datetime-template').find('.hr').val(hr);
						$fieldDiv.find('.updateFields-datetime-template').find('.min').val(min);
						$fieldDiv.find('.updateFields-datetime-template').find('.am-pm').val(am_pm);

						if (getProperty(v, 'locked', false)) {
							$fieldDiv.find('.updateFields-datetime-template').find('.hr').prop('disabled', 'disabled');
							$fieldDiv.find('.updateFields-datetime-template').find('.min').prop('disabled', 'disabled');
							$fieldDiv.find('.updateFields-datetime-template').find('.am-pm').prop('disabled', 'disabled');
						}
						else {
							$fieldDiv.find('.updateFields-datetime-template').find('.hr').removeProp('disabled');
							$fieldDiv.find('.updateFields-datetime-template').find('.min').removeProp('disabled');
							$fieldDiv.find('.updateFields-datetime-template').find('.am-pm').removeProp('disabled');
						}
					}
				}
				else {
					console.warn('Invalid ' + $fieldDiv.attr('data-type') + ' value for ' + fieldPath + ': ' + rawFieldInputData);
					$field.val('');
				}
			}
			else {
				$field.val(rawFieldInputData);
			}
