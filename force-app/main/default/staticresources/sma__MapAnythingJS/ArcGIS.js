var ArcGIS = {
    ...
    ajaxRequest: function(requestData) {
        ...
        Visualforce.remoting.Manager.invokeAction(MARemoting.processAJAXRequest, requestData, function(response, event) {
			if (response)
                ...
                dfd.resolve({
                    success: false,
					error: 'Memory error or time out.'
                });
        }, {
            buffer: false,
            escape: false,
    ...
    agolLogin: function(id, plotLayer, webMapURL) {
        ...
        window.open('https://www.arcgis.com/sharing/rest/oauth2/authorize' +
            ...
            '&response_type=token&' +
            '&redirect_uri=' + window.location.origin + '/apex/sma__MAArcGISRedirect?params=' + id + '_' + plotLayer + '_' + webMapHostURL,
            'MapAnything for ArcGIS',
            'scrollbars=yes, width=' + w + ', height=' + h + ', top=' + top + ', left=' + left);
    },
    serverRequest: function(layerId, url, webMapURL) {
        var dfd = jQuery.Deferred();
    ...
    plotLayer: function(options) {
        ...
        ArcGIS.createDomElement(options).then(function($layerHTML) {
            ...
            $layerHTML.find('.layername').html(MASystem.Labels.MA_Loading + '...');
            ...
            ArcGIS.ajaxRequest(requestData).then(function(response) {
                ...
                var layerOptions = JSON.parse(record.sma__Options__c);
                ...
                var url = layerOptions.baseURL;
                var automaticRefresh = layerOptions.automaticRefresh;
                var refreshInterval = layerOptions.refreshInterval == '--None--' ? 0 : layerOptions.refreshInterval;
                var found;
                if (found = url.match(/[?&](webmap|layers)=([0-9a-z,]+)/)) {
                    switch (found[1]) {
                        case 'webmap':
                            ArcGIS.plotHelpers.plotWebMap(layerId, found[2].split(',')[0], url, {
                                automaticRefresh: automaticRefresh,
                                interval: refreshInterval
                            });
                            break;
                        case 'layers':
                            ArcGIS.plotHelpers.plotWebMapLayers(layerId, found[2].split(','), url, {
                                automaticRefresh: automaticRefresh,
                                interval: refreshInterval
                            });
                            break;
                    ...
                    ArcGIS.plotHelpers.plotRESTLayer(layerId, url, {
                        automaticRefresh: automaticRefresh,
                        interval: refreshInterval
                    });
                }
		...
		if (ArcGIS.forceSecuredURLs && url && url.match(/^http:\/\//)) {
			...
			if (!ArcGIS.unsecuredServers[uniqueURL]) {
				...
				MAToastMessages.showWarning({
					message: 'Insecure URL <a onclick="copyToClipboard(this)">' + uniqueURL + '</a> requested. Layer will be requested over HTTPS, but it may not be displayed. Ensure the layer is hosted over HTTPS. Please share this warning and details with MapAnything Support and your ArcGIS Online Administrator.',
					timeOut: 0,
					extendedTimeOut: 0
				});
			}
                    ...
                    case 'Javascript':
                        ...
                        if ($button.attr('data-action') == 'Remove Marker') {
							...
							ArcGIS.ajaxRequest(requestData).then(function(response) {
                                var record = response.record;
								var templates = [];
								var arcC2Csettings = JSON.parse(getProperty(record, 'sma__ArcGISWebMapC2C__c')) || [];
                            ...
                            frameworkAction.ActionValue.call(this, {
                                button: $button,
                                dataLayers: layerObj,
                                records: $button.attr('data-action') == 'Street View' ? [records] : [],
    ...
    validateDetails: function() {
		...
		if (layerName == '' || layerName == '--None--') {
			...
			$('div#CreateNewArcGISWebMapPopup').find('input[name="Name"]').closest('.slds-form-element').append('<div id="arc-name-error-message" class="slds-form-element__help">This field is required.</div>');
			validDetails = false;
		}
		else {
			var longWords = false;
			...
			if (longWords) {
				$('#arc-name-error-message').remove();
				$('div#CreateNewArcGISWebMapPopup').find('input[name="Name"]').closest('.slds-form-element').addClass('slds-has-error');
				$('div#CreateNewArcGISWebMapPopup').find('input[name="Name"]').closest('.slds-form-element').append('<div id="arc-name-error-message" class="slds-form-element__help">Max word length is 20. Use whitespaces.</div>');
				validDetails = false;
			}
			else {
		...
		if (layerWebMapURL == '' || layerWebMapURL == '--None--') {
			...
			$('div#CreateNewArcGISWebMapPopup').find('textarea[name="WebMapURL"]').closest('.slds-form-element').append('<div id="arc-url-error-message" class="slds-form-element__help">This field is required.</div>');
			validDetails = false;
		}
		else {
		...
		if (automaticRefresh == '--None--') {
			...
			$('div#CreateNewArcGISWebMapPopup').find('input[name="AutomaticRefresh"]').closest('.slds-form-element').append('<div id="arc-automaticrefresh-error-message" class="slds-form-element__help">Select one setting.</div>');
			validDetails = false;
		}
		else {
        ...
        if (refreshInterval == '' || refreshInterval == '--None--' || (refreshInterval.match(/^([0-9]*\.)?[0-9]+$/) && refreshInterval > 0)) {
			...
			$('div#CreateNewArcGISWebMapPopup').find('input[name="RefreshInterval"]').closest('.slds-form-element').append('<div id="arc-refreshinterval-error-message" class="slds-form-element__help">This field must be > 0.</div>');
			validDetails = false;
		}
...
document.addEventListener("DOMContentLoaded", function() {
    $('#CreateNewArcGISWebMapPopup').on('click', "#newWebMapTemplate", function() {
        ...
        $('.arc-field-mapping-input #updateArcTemplate').attr('id', 'createArcTemplate').text('Create template');
        $("#webMapTemplateTable").removeClass('fadeInLeft');
    ...
    $('body').on('click', '#CreateNewArcGISWebMapPopup .edit-template', function() {
        ...
        ArcGIS.click2CreateHelpers.editTemplate($tempRow, currentWebMapOptions, ESRILayerInformation).then(function(rowData, updatedrow) {
            ...
            $('.arc-field-mapping-input #createArcTemplate').attr('id', 'updateArcTemplate').html('Update Template').removeAttr('disabled');
