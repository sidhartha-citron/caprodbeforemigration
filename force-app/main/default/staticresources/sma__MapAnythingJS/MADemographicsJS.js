var MAData = {
        ...
        create_step_2_V2 : function () {
            ...
            .done(function (data,textStatus,res) {
                ...
                if(!$popup.data('isMassAction')){
                    ...
                    //get data
                    var $status = showLoading($popup.find('.CreateRecordDataLayerPopup2'),MASystem.Labels.MA_Loading+'...');
                    MAData.createRecordFromDataLayer.getC2CDataFromServer(jsonData).then(function(res) {
                        ...
                        if(res.success) {
                            ...
                            for(var sfdcId in markerData) {
                                ...
                                try {
                                    ...
                                    if($input.length > 0) {
                                        if(!$popup.data('isMassAction')){
                                            // Date and Datetime need to be converted from UTC to something the date and time pickers can handle.
                                            if ($input.attr('data-type') == 'DATETIME' || $input.attr('data-type') == 'DATE') {
                                                var dateFormat = formatUserLocaleDate({datepicker: true}).replace('yy', 'YYYY').replace('mm', 'MM').replace('dd', 'DD');
                                                var localMoment = moment(fieldValue);
                                                localMoment.add(-1 * localMoment.utcOffset(), 'm');

                                                if (localMoment.isValid()) {
                                                    $input.find('.datepicker').val(localMoment.format(dateFormat));
                                                    
                                                    if ($input.attr('data-type') == 'DATETIME') {
                                                        var am_pm = localMoment.format('a').toUpperCase();
                                                        var hr = getProperty(MASystem, 'User.timeFormat').endsWith(' a') ? localMoment.format('hh') : localMoment.format('HH');
                                                        var min = localMoment.format('mm');
                                                        
                                                        $input.find('.updateFields-datetime-template').find('.hr').val(hr);
                                                        $input.find('.updateFields-datetime-template').find('.min').val(min);
                                                        $input.find('.updateFields-datetime-template').find('.am-pm').val(am_pm);
                                                    }
                                                }
                                                else {
                                                    console.warn('Invalid ' + $input.attr('data-type') + ' value for ' + sfdcId + ': ' + fieldValue);
                                                    $input.find('.get-input').val('');
                                                }
                                            }
                                            else {
                        ...
                        //if(!$popup.data('isMassAction')){
                            try { $popup.find('.createrecordDataLayer-fieldset .fieldInput[data-field="sma__Geolocation_Longitude__s"]').find('.get-input').val(markerInfo.markerData.lng); } catch (err) {}
                            try { $popup.find('.createrecordDataLayer-fieldset .fieldInput[data-field="sma__Geolocation_Latitude__s"]').find('.get-input').val(markerInfo.markerData.lat); } catch (err) {}
                            try { $popup.find('.createrecordDataLayer-fieldset .fieldInput[data-field="sma__MALongitude__c"]').find('.get-input').val(markerInfo.markerData.lng); } catch (err) {}
                            try { $popup.find('.createrecordDataLayer-fieldset .fieldInput[data-field="sma__MALatitude__c"]').find('.get-input').val(markerInfo.markerData.lat); } catch (err) {}
                        //}
    ...
    createC2CDLIcon: function (popupData, recordId) {
        ...
        var markerURL = MAIO_URL + '/services/images/marker?color=FF8800&forlegend=false&icon=Favorite';
    ...
    wizard: {
        ...
        resetPopup: function (keepHeader) {
            ...
            $('#MADataLayerEditor .dl-label').val('').removeClass('error');
            $('#MADataLayerEditor .dl-desc').val('');
            
            MAData.wizard.resetDataSource();
            MAData.wizard.resetTooltip();
        ...
        resetTooltip : function (checkForDefaults) {
            ...
            var detailId = $('#dl-level-select').val();
            if(sourceId == 'business' && checkForDefaults) {
                //leaving detail id out for now, currently '42' == 'businesses'
                var sampleData = MADemographicLayer.defaultDataSamples.business;
                //create headers
                var header = sampleData.headers
                for(var h = 0; h < header.length; h++) {
                    var hRow = header[h];
                    //create the row
                    MAData.wizard.createToolipRow({element:$('#dl-tooltip-header-addRow'),type:'header',data:hRow}); 
                }
                var tabs = sampleData.tabs;
                for(var t = 0; t < tabs.length; t++) {
                    var tab = tabs[t];
                    MAData.wizard.createTooltipTab(tab);
                }
            }
            else if (sourceId == 'property'  && checkForDefaults) {
                //leaving detail id out for now, currently '42' == 'businesses'
                var sampleData = MADemographicLayer.defaultDataSamples.property;
                //create headers
                var header = sampleData.headers
                for(var h = 0; h < header.length; h++) {
                    var hRow = header[h];
        ...
        launchPopup: function (options) {
            ...
            $popup.removeData();
            $popup.data('folderId',options.folderId);
            $popup.data('folderIdActual',options["folderIdActual"]);
            ...
            if(options.layerId) {
                ...
                $popup.data('layerId',options.layerId);
        ...
        getLayerInfo: function (layerOptions) {
            ...
            var $loadingSource = MAToastMessages.showLoading({message:MASystem.Labels.MA_Loading+'...',timeOut:0,extendedTimeOut:0});
            Visualforce.remoting.Manager.invokeAction(MARemoting.processAJAXRequest,
            	processData,
            	function(res, event){
            	    if(event.status) {
                       if(res.success) {
                           ...
                           if(sfLayer != undefined) {
                               ...
                               $popup.find('.dl-desc').val(sfLayer.sma__Description__c);
                                if(name.replace(/[ \t]/g, '') != '') {
                                ...
                                var jsonOptions;
                                try{
                                    jsonOptions = JSON.parse(sfLayer.sma__Options__c);
                                }
                                catch(e) {
        ...
        setupLegend:function () {
            ...
            var hasAutoLegend = $selectedOption.attr('has_auto_legend') == 'true' ? true : false;
            var $select = $('#dl-legendSelect');
        ...
        createLegend:function (type,options) {
            ...
            if(createLegend) {
                ...
                MA.Popup.showLoading({display:true,popupId:'MADataLayerEditor'});
                var $loadingSource = MAToastMessages.showLoading({message:MASystem.Labels.MA_Loading+'...',subMessage:'Grabbing legend information...',timeOut:0,extendedTimeOut:0});
        ...
        createLegendRow: function(options) {
            ...
            if((dataType == 'decimal' || dataType == 'currency') && !options.isOther) {
        ...
        updateValueField: function (select,forLegend) {            
            ...
            if(operator == 'is not null' || operator === 'is null') {
                //This operator does not use value input from the user.
                $row.find('.dl-filter-value-wrapper').hide();
                $row.find('.value').hide();
            }
            else {
                $row.find('.dl-filter-value-wrapper').show();
                $row.find('.value').show();
                var dataType;
        ...
        buildOperatorSelect: function (type,forLegend) {            
            ...
            operatorSelectHTML += '<option value="is not null">is not null</option>';
            operatorSelectHTML += '<option value="is null">is null</option>';
        ...
        saveDataLayer:function (plotLayer) {
            ...
            if($('#MADataLayerEditor').data('layerId') && !isClone) {
                layerId = $('#MADataLayerEditor').data('layerId');
            }
            else {
                if($('#MADataLayerEditor').data('folderId') == 'PersonalRoot' || $('#MADataLayerEditor').data('folderId') == 'RoleUserFolder') {
                    if($('#MADataLayerEditor').data('folderId') == 'RoleUserFolder') {
                        if (NewLayerNavigationEnabled()) {
                            //not supported right now
                            userId = MA.CurrentUser.Id;
                        }
                        else {
                            userId = $popup.data('folderIdActual');
                        }
                ...
                else if ($('#MADataLayerEditor').data('folderId') == 'CorporateRoot') {
                    //send nothing for corporate root
                }
                else {
                    //Something has to happen if we're in a subfolder here
                    
                    folderId = $('#MADataLayerEditor').data('folderId');
                    var breadcrumbText = '';
            ...
            var processData = { 
            	...
            	layerData: JSON.stringify({
                    ...
                    sma__Folder__c : folderId === 'Home' ? '' : folderId,
            	    ...
            	    if(event.status) {
                        ...
                        if(res.success) {
                            ...
                            if (NewLayerNavigationEnabled())
                            {
                                ...
                                MALayers.refreshFolder().always(function() {
                                    if(plotLayer) {
                                        if(isRefresh) {
                                            //refresh
                                            ...
                                            MADemographicLayer.analyzeDataLayer({isPlotted:isPlotted,id:dataId,edit:true,modify:true,read:true,delete:true,name:(res.data.Name || 'Saved Data Layer')});
                                        }
        ...
        validateDataLayer : function () {
            ...
            //validate our name
            var $dataName = $popup.find('.dl-label').removeClass('error');
            if($dataName.val().replace(/[\t]/g, '') == '') {
                //go to tab
                $('#dl-details').click();
                //add error Class
                $dataName.addClass('error');
            ...
            $('#dlFilters .dl-filter-row').each(function(i,row) {
                ...
                if($rowOperator.val() == null || $rowOperator.val() == '--') {
                ...
                else if($rowOperator.val() == 'is null' || $rowOperator.val() == 'is not null') {                    
                    filter['values'] = null;
                } else {
            ...
            if(filterError) {
                MAToastMessages.showError({message:'Data Layer Save Error.',subMessage:'Please check all filters and update missing values.',timeOut:0,closeButton:true});
                $('#dl-filtersFix').click();
                return {success:false};
            }
    ...
    getDataLayerSource: function(urlParameters,updateSelect) {
        ...
        *******************/
        MA.Popup.showLoading({display:true,popupId:'MADataLayerEditor'});
        var $loadingSource = MAToastMessages.showLoading({message:MASystem.Labels.MA_Loading+'...',subMessage:'Grabbing latest info...',timeOut:0,extendedTimeOut:0});
        ...
        if (userSettings.SelfHostedDataUser == true && $("#uploader-link").length !== 1) {
            $('#dl-grid').append(
                '<a id="uploader-link" href="https://app.mapanything.io/" target="_blank"'
                + 'style="'
                + 'padding-left: .75rem; '
                + 'color: #005fb2; '
