...
var MAListView = {
	...
	logCallHTML: actionHandler == 'showPopup'?'<a onclick="logACall_First([\'::id::\']);"><span aria-hidden="true" class="MAIcon glyphicon-phone-alt"></span></a>' : '<a href="sma__MapActions?action=log_call&id=::id::" target="_blank"><span aria-hidden="true" class="MAIcon glyphicon-phone-alt"></span></a>',
	...
	ListViewColumns: { 
		...
		'3'		: { "label":MASystem.Labels.MAActionFramework_Log_a_Call,	"visible":"true", "id":"3", "deprecated" : false,	'sortColumn' : null, "isRecordProperty" : false },
					...
					switch(col.id)
					{
						...
						case '2':  
							...
							else 
							{
                                ...
                                var imgId = brush.split(':')[1];
                                var checkImgIdForListView = function() {
                                    if(imgId.substring(0, 3) === '015') {
                                        return '/servlet/servlet.FileDownload?file=' + imgId
                                    }
                                    return '/sfc/servlet.shepherd/version/download/' + imgId
                                }
                                switch(markerAssignmentType) 
                                {
                                    case 'Static':
                                        if(brush.indexOf('image') >= 0) {
                                            colContents += '<img src="' + MA.SitePrefix + checkImgIdForListView() + '" class="listview-marker-icon" />';
                                        } else {
                                            var colorParts = brush.split(':');
                                            var iconPart = colorParts[1] || 'Circle';
                                            var iconColor = colorParts[0] || '3083d3';
                                            iconColor = iconColor.replace(/#/g,'');
                                            var markerURL = MAIO_URL + '/services/images/marker?color='+iconColor+'&forlegend=true&icon='+iconPart;
                                            colContents += '<img src="'+markerURL+'" style="width:12px;" />';
                                            // colContents += MAMarkerBuilder.createSVG({ color : record.markerValue, forLegend : true });
                                        }
						...
						//FULL ADDRESS 
						case '21':
							colContents = MAPlotting.getFormattedAddress(record, layerData) || '';
						...
						case '25':
							colContents = isSelectedTab ? 'N/A' : colContents;
	...
	DrawTabTableDataRows: function(options) {
		...
		setTimeout(function doBatch() {
            if ((recordsDisplayed < options.listViewSettings.pageSize) && (startIndex < recordsListLength))
            {
                var recProcessed = 0;
                while ((recProcessed < 100) && (startIndex < recordsListLength) && (recordsDisplayed < options.listViewSettings.pageSize)) {
                    recProcessed++;
	...
	ExportTab: function (layerId, filename)
	{
		...
		if (isSelectedTab)
		{
			recordCount = MAListView.recordList.length;
		}
		else
		{
			loadingDone = false;
		...
		var loadingInt = setInterval(function ()
		{
			if (loadingDone)
			{
                ...
                var popupNotify = 'This action will require a popup. Please ensure popups are allowed for this site.';
                var confirmPop = MA.Popup.showMAConfirm({
                    template: '<div style="text-align:center;" div><div>Are you sure you want to export ' + exportSize + ' records?</div><br><div>**You may experience a temporary slow down.</div><div style="color: #c23934;">**' + popupNotify + '</div></div>'
                });
	...
	UpdatePageShowing: function(options) {
				...
				setTimeout(function doBatch() {
					if (recCount < recLengh) {
						...
						while (recordsProcessed < markerProcessingBatchSize && recCount < recLengh) {
							...
							var record = records[recId];
							if(record) {
								var isVisible = record.listViewVisible === true ? true : record.isVisible || record.isScattered || record.isClustered;
								if(isVisible) {
									total++;
								}
							}
		...
		var countInterval = setInterval(function () {
			if(loadingDone) {
				...
				var unfiltered = isFiltered ? '(' + recordList.length + ' ' + MASystem.Labels.MA_Markers_Created + ')' : '';
		
				
				
				countHTML = MASystem.Labels.MA_Showing + ' ' + start + '-' + actualEnd + ' ' + MASystem.Labels.MA_of + ' ' + total + '.' + unfiltered;
		    ...
		    if(!options.rawValue) {
                ...
                if(options.toolTipIndex === 0 || options.layerData.tooltips[options.toolTipIndex].ActualFieldName === 'WorkOrderNumber') {
        			...
        			if (typeof tooltipData === 'string') {
        				if( tooltipData.indexOf('&lt;a') === 0 ) {
        					colContents = tooltipData.replace('_top', '_blank').replace('_self', '_blank').replace('_parent', '_blank');
						} else if (tooltipData.indexOf('&lt;img') > -1) {
							colContents = htmlDecode(tooltipData);
						} 	
                    }
                }
	...
	GetColumnValue: function(layer, recordId, colId, isSelectedTab, raw) {
	    ...
	    switch(colId) {
    		...
    		//COMBINED LAT LNG	
    		case '22':
    			if (lat != 'N/A') { colContents = parseFloat( record.marker.getPosition().lat() ).toFixed(4) + ',' + parseFloat( record.marker.getPosition().lng() ).toFixed(4); }
				colContents = htmlEncode(colContents);
				break;
	...
	ClearAllSelected: function() {
		...
		if ($(MAListView.queryClass).length) {
			...
			$('.listview-data-table').find('input[type="checkbox"]').prop('checked', false);
	...
	ClearSelected: function(layerId) {
	    ...
	    if( mainCheckBox.prop("checked") ) {
			...
			$('div#' + layerId).find('.with-selected-wrapper').text(MASystem.Labels.MA_with + ' (0) ' + MASystem.Labels.MA_selected);
	...
	SelectAll: function(layerId) {
		...
		$('div#' + layerId).find('.with-selected-wrapper').text(MASystem.Labels.MA_with + ' (' + layerData.listViewSettings.selectedIds.length + ') ' + MASystem.Labels.MA_selected);
	...
	ShowFilterPopup: function(options) {
		...
		*/
		...
		rowHTML = '<tr>';
		//	rowHTML += '<td></td>';
		rowHTML += '<td><label class="slds-form-element__label">' + 'All Columns' + '</label></td>';
		rowHTML += '<td><label class="slds-form-element__label">' + 'Contains' + '</label></td>';
		...
		$.each(userSettings.ListViewColumns, function( index, colId ) {
		   ...
		   if (col.visible == 'true')
		   {
			   ...
			   else if ($.inArray( col.id, [ '21' ] ) > -1)
			   {
				 ...
				 //  rowHTML += '<td></td>';
				   rowHTML += '<td><label class="slds-form-element__label">' + 'Street' + '</label></td>';
				   rowHTML += '<td>' + '<div class="slds-select_container"><select class="listViewOperator slds-select">' +  MAListView.operators.typeString.join('') +   '</select></div>' + '</td>';
				   rowHTML += '<td>' + '<input type="text" class="value-input slds-input" value="' + currentValue + '"/>' + '</td>';
				   rowHTML += '</tr>';
				   ...
				   //rowHTML += '<td></td>';
				   rowHTML += '<td><label class="slds-form-element__label">' + 'City' + '</label></td>';
				   rowHTML += '<td>' + '<div class="slds-select_container"><select class="listViewOperator slds-select">' +  MAListView.operators.typeString.join('') +   '</select></div>' + '</td>';
				   rowHTML += '<td>' + '<input type="text" class="value-input slds-input" value="' + currentValue + '"/>' + '</td>';
				   rowHTML += '</tr>';
				  ...
				  // rowHTML += '<td></td>';
				   rowHTML += '<td><label class="slds-form-element__label">' + 'State' + '</label></td>';
				   rowHTML += '<td>' + '<div class="slds-select_container"><select class="listViewOperator slds-select">' +  MAListView.operators.typeString.join('') +   '</select></div>' + '</td>';
				   rowHTML += '<td>' + '<input type="text" class="value-input slds-input" value="' + currentValue + '"/>' + '</td>';
				   rowHTML += '</tr>';
				  ...
				  // rowHTML += '<td></td>';
				   rowHTML += '<td><label class="slds-form-element__label">' + 'Postal Code' + '</label></td>';
				   rowHTML += '<td>' + '<div class="slds-select_container"><select class="listViewOperator slds-select">' +  MAListView.operators.typeString.join('') +   '</select></div>' + '</td>';
				   rowHTML += '<td>' + '<input type="text" class="value-input slds-input" value="' + currentValue + '"/>' + '</td>';
				   rowHTML += '</tr>';
				   ...
				   //rowHTML += '<td></td>';
				   rowHTML += '<td><label class="slds-form-element__label">' + 'Country' + '</label></td>';
				   rowHTML += '<td>' + '<div class="slds-select_container"><select class="listViewOperator slds-select">' +  MAListView.operators.typeString.join('') +   '</select></div>' + '</td>';
				   rowHTML += '<td>' + '<input type="text" class="value-input slds-input" value="' + currentValue + '"/>' + '</td>';
				   rowHTML += '</tr>';
			   }
			   else if ($.inArray( col.id, [ '22' ] ) > -1)
			   {
				   ...
				   //rowHTML += '<td></td>';
				   rowHTML += '<td><label class="slds-form-element__label">' + 'Longitude' + '</label></td>';
				   ...
				   //rowHTML += '<td></td>';
				   rowHTML += '<td><label class="slds-form-element__label">' + 'Latitude' + '</label></td>';
				   rowHTML += '<td>' + '<div class="slds-select_container"><select class="listViewOperator slds-select">' +  MAListView.operators.typeDecimal.join('') +   '</select></div>' + '</td>';
				   rowHTML += '<td>' + '<input type="text" class="value-input slds-input" value="' + currentValue + '"/>' + '</td>';
				   rowHTML += '</tr>';
			   ...
			   else if ($.inArray( col.id, [ '5' ] ) > -1 && options.layerId == 'SelectedTab')
			   {
				 ...
				 //  rowHTML += '<td></td>';
				   rowHTML += '<td><label class="slds-form-element__label">' + MAListView.ListViewColumns[col.id].label + '</label></td>';
				   rowHTML += '<td>' + '<div class="slds-select_container"><select class="listViewOperator slds-select">' +  MAListView.operators.typeDecimal.join('') +   '</select></div>' + '</td>';
				   rowHTML += '<td>' + '<div class="slds-form-element slds-form_compound"><div class="slds-form-element__row"><div class="slds-form-element"><label class="slds-form-element__label" for="listview-days">Days</label><div class="slds-form-element__control"><input type="text" class="days slds-input" value="' + currentValue + '" id="listview-days"/></div></div><div class="slds-form-element"><label class="slds-form-element__label" for="listview-hours">Hours</label><div class="slds-form-element__control"><input type="text" class="hours slds-input" id="listview-hours"/></div></div><div class="slds-form-element"><label class="slds-form-element__label" for="listview-minutes">Minutes</label><div class="slds-form-element__control"><input type="text" class="minutes slds-input" id="listview-minutes"/></div></div></div></div>  ' + '</td>';
				   rowHTML += '</td>';
				   rowHTML += '</tr>';
			   }
	...
	clearFilterPopup: function() {
		...
		$('#listview-filter-popup .listViewOperator').val('');
		...
		*/
		...
		if (options.doSave)
		{
			...
			$( "tr.listViewFilterRow" ).each(function( index ) {
				...
				if (operator != 'none' && colValue != '' && colValue != undefined && colValue != null)
				{
