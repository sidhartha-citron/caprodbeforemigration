...
try {
            ...
            function(response, event){
                ...
                if(event.status) {
                    ...
                    else if (response.fieldType == 'ID')
                    {
                        ...
                        var cacheFrag = document.createDocumentFragment();
                    	$.each(response.cachedQueryOptions || [], function (i, opt) {
                            var optFrag = document.createElement('option');
                            optFrag.value = opt.value;
                            optFrag.innerHTML = opt.label;
                            cacheFrag.appendChild(optFrag);
                        });
                        $cachedQueryOptions[0].appendChild(cacheFrag);
                    	$cachedQueryOptions.trigger('change.select2');
            ...
            function(response, event){
                ...
                if(event.status) {
                    ...
                    if (tooltipData.parentFieldOptions && tooltipData.parentFieldOptions.length > 0)
                    {
                        ...
                        //add empty option
                        var emptyLabel = '--Select a Field--';
                        var optionFrag = document.createElement('option');
                        optionFrag.value = emptyLabel;
                        optionFrag.textContent = emptyLabel;
                        bulkFrag.appendChild(optionFrag);
    ...
    function buildQuery(options)
    {
        ...
        var queryObj = {
            ...
            useFilterLogic      : $('#savedqueryeditor .filterlogiclink').text() == MASystem.Labels.MA_Add ? false : true,
        ...
        $('#savedqueryeditor .tooltip').each(function () {
            ...
            if(isLookup == 'true') {
                ...
                if(childValue == '--Select a Field--' || childValue == null) {
                    ...
                    tooltipField = parentValue + '.' + childValue;
                }
            }
            else {
    ...
    ************************/
    function RelatedList_Add()
    {
    	...
    	setTimeout(function () {
            $('#relatedlist-addnew-object').val('--').next().find('input').val('--'+MASystem.Labels.MA_Add_New+'--');
            $('#select2-relatedlist-addnew-object-container').text('--'+MASystem.Labels.MA_Add_New+'--');
    	}, 500);
    }
    ...
    {
    	//create comboboxes
    	$relatedList.find('.combobox').select2();
        ...
        var processData = {
            ...
            baseObject: query.baseObject,
            crossObjectName: $relatedList.data('crossObjectName')
        };
            ...
            function(response, event){
                ...
                if(event.status) {
                	...
                	if (relatedList)
                	{
                		...
                		$relatedList.find('.relatedlist-orderby').val(relatedList.relatedlistsort.split(' ')[0]).next().find('input').val($relatedList.find('.relatedlist-orderby option:selected').text()).trigger('change.select2');
                		$relatedList.find('.relatedlist-orderby-direction').val(relatedList.relatedlistsort.split(' ')[1]).next().find('input').val($relatedList.find('.relatedlist-orderby-direction option:selected').text()).trigger('change.select2');
                        
                        $relatedList.find('.relatedlist-numtodisplay').trigger('change.select2');
                        $relatedList.find('.relatedlist-orderby').trigger('change.select2');
                		$relatedList.find('.relatedlist-orderby-direction').trigger('change.select2');
                		//label
    ...
    {
    	//add column
        var $column = $('<div class="inline relatedColumnWrap"><select style="width:155px;" class="relatedlist-column"><option value="--">--'+MASystem.Labels.MA_None+'--</option></select></div>');
        var orderFrag = document.createDocumentFragment();
        ...
        var $columSelect = $column.find('.relatedlist-column');
        $columSelect[0].appendChild(orderFrag);
        $column.insertBefore($relatedList.find('.relatedlist-addcolumn'));
    ...
    $(function () {
        ...
        $('#savedqueryeditor')
            ...
            .on('click', '.markertype-color-preview', function (e) {
                ...
                var $bubble = $('#savedqueryeditor-templates .markerpicker').clone()
                    .addClass('bubble top')
                    .css({ position: 'absolute', top: offsetTop, left: offsetLeft, width: 200, height: 265, padding: '5px' })
                    .appendTo('body')
                        ...
                        try {
                            ...
                            $.each(MAMarkerBuilder.shapes, function (shape, shapeMeta) {
                                if ($.inArray('dynamic', shapeMeta.types) != -1) {
                                    var svg = MAMarkerBuilder.createSVG({ color: hex + ':' + shape});
                                    html += '<td>'+svg+'</td>'
                                }
                            });
                            html += '</tr></table>';
                ...
                $shapeWrapper.on('click', 'svg', function () {
                    ...
                    if ($preview.is('.massaction')) {
                    	$preview.closest('.markergrid').find('.markergrid-row .markertype-color-preview').attr('data-color', color).html(MAMarkerBuilder.createSVG({ color: color, forLegend: true }));
                    }
                    else {
        ...
        var processData = {
            ...
            action: 'getSortableFieldOptions',
            baseObject: query.baseObject
        };
            ...
            function(response, event){
                ...
                if(event.status) {
                    ...
                    $.each(response.data.sortableFieldOptions || [], function (index, option) {
                        $('<option></option>').attr('value', option.itemValue).text(option.itemLabel || option.itemValue).appendTo($sortableField);
                    });
            ...
            function(response, event){
                ...
                if(event.status) {
                    ...
                    $.each(response.data.tooltipOptions || [], function (index, option) {
                        ...
                        if(option.fieldDisplayType == 'reference') {
                            option.itemLabel = option.itemLabel + ' (Lookup)';
                            isReference = 'true';
                        }
                    ...
                    if ($tooltips.data('baseobject') == 'Case') {
                        $($tooltips[0]).val('CaseNumber');
                    } else if ($tooltips.data('baseobject') == 'WorkOrder'){
                        $($tooltips[0]).val('WorkOrderNumber');
                    } else {
                        $($tooltips[0]).val('Name');
                    }
                    ...
                    $.each(response.data.picklistOptions || [], function (index, option) {
                        ...
                        $('<option></option>').attr('value', option.itemValue).attr('data-lookup','baseobject field').text(option.itemLabel).appendTo($picklistField);
                        if(option.fieldDisplayType == 'reference')
                            {                                    
                                $('<option></option>').attr('value', option.itemValue).attr('data-lookup',"lookup").text(option.itemLabel + ' (Lookup)').appendTo($picklistField);                              }
                    		});
                    /**End March Release*/
                    ...
                    var $relatedListOptions = $('#relatedlist-addnew-object').html('<option value="--">--'+MASystem.Labels.MA_Add_New+'--</option>');
        ...
        *****************************/
                    ...
                    function(response, event){
                        ...
                        if(event.status) {
                            ...
                            else
                            {
                                ...
                                $colorPicklistTR.find("#advanced-reference-option-automaticassign").attr('checked',false);
                            }
		...
		$('.color-picklistparentfield').change(function () {
        	...
        	else
        	{
        	    ...
        	    $colorPicklistParentTR.find("#advanced-reference-option-automaticassign").attr('checked',false);
        	}
        ...
        $('#savedqueryeditor').on('change', '.multiField', function () {
            ...
            if(queryType.toLowerCase() == 'dynamic-label') {
                ...
                if(picklistfield != null && picklistfield.indexOf('.') > -1 && !$comboBox.data()) {  
        ...
        $('#savedqueryeditor .savebtn').click(function (event) //teddy - added event param
        {
            ...
            //make sure there are no invalid limits
           	try {
           		...
           		if ($('.limit-records-enabled').attr('checked') == 'checked' && (isNaN(rowLimit) || rowLimit < 1 || rowLimit > 250000)) {
            ...
            try {		
           	...
           	{
           		MAToastMessages.showWarning({message: MASystem.Labels.MA_Validation_Error,subMessage: MASystem.Labels.MA_Validation_Error_Caching_interval,timeOut:8000,closeButton:true});
               	return;
           	}
           	...
           	{
           		MAToastMessages.showWarning({message: MASystem.Labels.MA_Validation_Error,subMessage: MASystem.Labels.MA_Validation_Error_Caching_owner,timeOut:8000,closeButton:true});
               	return;
           	}
            ...
            if ($('[data-tooltip="tooltip1"]').val() === '--'+MASystem.Labels.MA_None+'--' || $('[data-tooltip="tooltip1"]').val() === null)
            {
			...
			$('#savedqueryeditor table.markertable .multipicklistvalue').each(function()
			{
			 ...
			 if($('.color-assignmenttype').val() == 'Dynamic-multiField')
			 {
			     ...
			     else if($('.shapemarkergrid-header').attr('data-type') == 'DATE')
			     {
			        ...
			        //  $('.shapemarkergrid .shapemarkergrid-row').each(function(){
			        //      if(markerRulesValid){ 
			        //         if($(this).find('.comparevalue').val() == '' || $(this).find('.enddate').val() == '') { 
		            //     	    markerRulesValid = false;
		                	    
		                	    
		            //         }
		            //         if(($(this).find('.comparevalue').val().indexOf('/') > -1 && $(this).find('.enddate').val().indexOf('/') < 0)  || ($(this).find('.enddate').val().indexOf('/') > -1 && $(this).find('.comparevalue').val().indexOf('/') < 0)) { 
		            //     	    markerRulesValid = false;
			        ...
			        //  if(!markerRulesValid)
                    //  { 
                    //     //$('#savedqueryeditor .buttons .msgs').html("<bMASystem.Labels.MA_Validation_Error/b>: You must enter a valid FROM-TO range for markers before proceeding.").show();  
                    //     MAToastMessages.showWarning({message:MASystem.Labels.MA_Validation_Error,subMessage:'You must enter a valid FROM-TO range for markers before proceeding',timeOut:8000,closeButton:true});
                	//     return false;
                    //  }
			     }
            ...
            {
            	MAToastMessages.showWarning({message: MASystem.Labels.MA_Validation_Error,subMessage:'Distance filtering and caching cannot currently be used together',timeOut:8000,closeButton:true});
            	return false;
            }
        ...
        if (query.id || query.isClone)
        {
           ...
           if(markerShapeType == 'static') {
            ...
            if (query.cacheInterval != null) {
                try {
           			$('#savedqueryeditor .advanced-enablecaching').prop('checked', true);
