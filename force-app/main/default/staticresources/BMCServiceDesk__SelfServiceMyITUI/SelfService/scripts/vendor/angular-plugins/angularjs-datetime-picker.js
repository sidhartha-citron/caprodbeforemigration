(function() {
  ...
  var DatetimePickerCtrl = function($compile, $document) {
    ...
    this.openDatetimePicker = function(options) {
	  ...
	  datetimePickerEl.style.zIndex = "1100";
  ...
  var tmpl = [
    ...
    '  </div>',
    '  <div class="adp-days" id="adp-time" style="display:none;"> ',
    ...
    var linkFunc = function(scope, element, attrs, ctrl) { //jshint ignore:line
      ...
      scope.$applyAsync( function() {
        ...
        if (!scope.selectedDate || isNaN(scope.selectedDate.getTime())) { // no predefined date
          ...
          var hour = (typeof(scope.hour) != 'undefined' && scope.hour != null && scope.hour >= 0) ? scope.hour : today.getHours();
          var minute = (typeof(scope.minute) != 'undefined' && scope.minute != null && scope.minute >= 0) ? scope.minute : today.getMinutes();
      ...
      scope.updateNgModel = function(day) {
        ...
        if (attrs.ngModel) {
          ...
          if (elScope.$eval(attrs.ngModel) && elScope.$eval(attrs.ngModel).constructor.name === 'Date') {
            ...
            dateValue = dateFilter(scope.selectedDate,dateFormat).replace('a.m.', 'AM').replace('p.m.','PM').replace('午前', 'AM').replace('vorm.', 'AM').replace('午後', 'PM').replace('nachm.', 'PM'); 
          }
