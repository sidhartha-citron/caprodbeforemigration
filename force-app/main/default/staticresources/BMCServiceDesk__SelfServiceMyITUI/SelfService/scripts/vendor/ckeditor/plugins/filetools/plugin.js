	...
	CKEDITOR.plugins.add( 'filetools', {
		...
		beforeInit: function( editor ) {
			...
			editor.on( 'fileUploadRequest', function( evt ) {
				...
				fileLoader.xhr.setRequestHeader("Access-Control-Allow-Origin", "*");
                ...
                for ( var name in requestData ) {
                    ...
                    if ( typeof value === 'object' && value.file ) {
                        ...
                        var fileBody = fileLoader.data.substr(fileLoader.data.indexOf(',') + 1);
                        Visualforce.remoting.Manager.invokeAction(_RemotingActions.uploadImage, fileBody, fileName, param.imageType, function(result, event) {
                            if (event.status) {
                                if (result != null) {
                                    fileLoader.responseData = '';
                                    fileLoader.url = getSFDocumentURL(relativeServletURL + result);
                                    fileLoader.changeStatus( 'uploaded' );
                                } else {
                                    fileLoader.message = param.FileUploadFailed;
